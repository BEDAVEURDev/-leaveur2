    ...
    async def leaveur(self, ctx, channel: discord.TextChannel, *, message):
        ...
        if formatted_message:
            ...
            await self.db.find_one_and_update(
                {'_id': 'config'},
                {'$set': {'leaveur': {'channel': str(channel.id), 'message': message}}},
                upsert=True
            )
        ...
        if config:
            ...
            if channel:
                ...
                if message:
                    ...
                    await channel.send('Invalid leaveur message')
            else:
                print('leaveur plugin not found: {getattr(channel, "id", None}')


def setup(bot):
    bot.add_cog(leaveur(bot))
